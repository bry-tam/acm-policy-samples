...
### Recent changes:
  > - (Aug 2025)
  >    - Add READMEs to every policy noting ACM version required, link to documentation, any dependencies and other notes of interest
  >    - Deploy all OLM Operators with `OperatorPolicy`
  >    - Add build steps to validate PolicyGenerators build, yaml is validated against CRDs, all files end in empty line, there are no trailing whitespace, all yaml includes the --- doc statement
  >    - All kustomization configs were moved to a common folder and included as a Component in kustomize.
  >    - Docs added to outline policy standards and list of policies within the repo (only Operators added in this update)
  >    - template-examples added to give a home to random template examples that don't need a full policy to be shared/used.
...
## Build Rules
The repo also contains CI scripts used to enforce policy and yaml linting rules.  These can be found in the ./build and ./.github directories.  The rules applied are outlined in [docs/policy-prerequisites.md](docs/policy-prerequisites.md) file.
